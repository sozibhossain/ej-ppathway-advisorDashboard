    ...
    <div className="space-y-5">
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        <div className="flex flex-wrap items-center gap-2">
          <Button variant="secondary" className="bg-white text-slate-700 border border-slate-200 hover:bg-slate-50">
          ...
          <Button onClick={() => markUnavailable(true)}>
            <PlusIcon size={15} />
            Add Time Off
          </Button>
        </div>
        ...
        <aside className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
          ...
          <div className="mt-5 flex items-center justify-between">
            <div>
              ...
              <p className="mt-0.5 text-xs text-slate-500">
                {selectedHasOverride
                  ? "Edits apply only to this date."
                  : "This date currently follows your weekly schedule."}
              </p>
            </div>
          ...
          <div className="mt-4 rounded-lg border border-slate-200 bg-slate-50 p-3">
            ...
            <div className="mt-2 space-y-1">
              {selectedWeeklySlots.length ? (
                selectedWeeklySlots.map((slot, index) => (
          ...
          <div className="mt-4 space-y-3">
            <div className="flex items-center justify-between">
              <h3 className="text-sm font-bold text-slate-900">
                {selectedHasOverride ? "This date override" : "Visible slots"}
              </h3>
              {selectedHasOverride ? (
                <button
                  type="button"
                  onClick={clearDateOverride}
                  className="text-xs font-bold text-[#0a7a90] hover:underline"
                >
                  Use weekly schedule
                </button>
              ) : (
            ...
            {selectedRule.unavailable ? (
              ...
              selectedSlots.map((slot, index) => (
                <div key={`${selectedDate}-${index}`} className="flex items-center gap-2 rounded-lg border border-slate-200 px-3 py-2">
                  ...
                  <button type="button" className="ml-auto text-slate-400 hover:text-[#0a7a90]" aria-label="Edit time slot">
                    <PencilIcon size={15} />
                  </button>
                  <button type="button" onClick={() => removeSlot(index)} className="text-slate-400 hover:text-red-600" aria-label="Remove time slot">
                    <TrashIcon size={15} />
                  </button>
                </div>
              ))
          ...
          <div className="mt-6 border-t border-slate-100 pt-4">
            ...
            <div className="mt-3 grid grid-cols-2 gap-2">
              ...
              <ActionButton icon={<CopyIcon size={15} />} label="Copy to Week" onClick={copyToWeek} />
              <ActionButton icon={<ClockIcon size={15} />} label="Set Recurring" onClick={addWeeklySlot} />
              <ActionButton icon={<CloseIcon size={15} />} label="Mark as Unavailable" danger onClick={() => markUnavailable(true)} />
            </div>
          </div>
          ...
          <Button onClick={save} loading={saving} className="mt-5 w-full">
            Save Changes
          </Button>
        </aside>
      </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/50 p-4">
          <div className="w-full max-w-md rounded-2xl border border-slate-200 bg-white p-5 shadow-2xl">
            ...
            <div className="mt-5 rounded-lg bg-slate-50 p-3">
              ...
              <div className="mt-2 space-y-1">
                {selectedSlots.length ? (
                  selectedSlots.map((slot, index) => (
                    <div key={`${selectedDate}-copy-${index}`} className="text-sm font-semibold text-slate-700">
                      {slot.from} - {slot.to}
                    </div>
                  ))
                ) : (
            ...
            <div className="mt-5 flex justify-end gap-2">
              ...
              <Button type="button" onClick={applyToAnotherDate} disabled={selectedSlots.length === 0}>
                Apply Override
              </Button>
            </div>
          </div>
